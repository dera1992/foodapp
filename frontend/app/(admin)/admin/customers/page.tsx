  ...
  return (
    <TableCard title="Customers">
      <table className="min-w-full text-sm">
        ...
        <tbody>{customers.map((customer) => (<tr key={customer.id} className="border-t border-brand-border"><td className="py-2">{customer.name}</td><td>{customer.orders}</td><td className="text-right">£{(customer.spend ?? 0).toFixed(2)}</td></tr>))}</tbody>
      </table>
    </TableCard>
  );
}